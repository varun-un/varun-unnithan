...
const onSceneReady = (scene) => {
    ...
    var camera = new BABYLON.ArcRotateCamera("arcCamera", Math.PI / -2, Math.PI / 2, 7, BABYLON.Vector3.Zero(), scene);
    ...
    function logKey(e) {
        ...
        camera.alpha = 1 * xLoc * BABYLON.Tools.ToRadians(10) + (Math.PI / -2);
        camera.beta = 1 * yLoc * BABYLON.Tools.ToRadians(10) + (Math.PI / 2);
    }
...
const onRender = (scene) => {
    if (torus !== undefined) {
        ...
        torus.rotation.z = BABYLON.Tools.ToRadians(zRotCos(frame, 5, 400));
    }
};
...
function App(name) {
    return (
        <div>
            <SceneComponent class='nameBabylon' antialias onSceneReady={onSceneReady} onRender={onRender} id="my-canvas" />
        </div>
    )
}
