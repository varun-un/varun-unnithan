...
const onSceneReady = (scene) => {
    ...
    var camera = new BABYLON.ArcRotateCamera("arcCamera", Math.PI / -2, Math.PI / 2, k / window.innerWidth + .5, BABYLON.Vector3.Zero(), scene);
    camera.attachControl(canvas, true);
    camera.angularSensibilityX = 1500;
    camera.angularSensibilityY = 1500;
    camera.lowerBetaLimit = (0);
    camera.upperBetaLimit = (2 * Math.PI / 3);
    camera.upperRadiusLimit = k / window.innerWidth + .5;
    camera.lowerRadiusLimit = k / window.innerWidth + .5;
